...
def explain_analyze(query: str) -> str:
    ...
    if not any(query_upper.startswith(keyword) for keyword in allowed_keywords):
        return ("Error: Only TABLE, MATERIALIZED VIEW, SINK, INDEX, or ID queries "
                "are allowed for EXPLAIN ANALYZE")
...
def explain_query(query: str) -> str:
    ...
    if not any(query_upper.startswith(keyword) for keyword in allowed_queries):
        return "Error: Only SELECT, INSERT, UPDATE, DELETE, and WITH queries are allowed for EXPLAIN"

    dangerous_keywords = ['DROP', 'TRUNCATE', 'ALTER', 'CREATE']
    if any(keyword in query_upper for keyword in dangerous_keywords):
        return "Error: Queries containing DDL operations are not allowed"

    rw = setup_risingwave_connection()
    try:
        explain_q = f"EXPLAIN {query}"
        result = rw.fetch(explain_q, format=OutputFormat.DATAFRAME)
        return result.to_json()
    except Exception as e:
...
def explain_distsql(statement: str, verbose: bool = False) -> str:
    ...
    if not any(statement_upper.startswith(prefix) for prefix in allowed_prefixes):
        return ("Error: Only CREATE MATERIALIZED VIEW, CREATE SINK, CREATE INDEX, CREATE TABLE, "
